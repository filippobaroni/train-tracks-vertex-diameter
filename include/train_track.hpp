...
public:
    ...
    explicit TrainTrack(size_t switches_count = 0, size_t branches_count = 0) : switches(switches_count),
    ...
    [[nodiscard]] const auto& get_switches() const;
    [[nodiscard]] const auto& get_branches() const;
    [[nodiscard]] const auto& get_surface() const;

    template <std::integral T>
    std::vector<Measure<T>> get_vertex_measures() const;

    template <std::integral T>
    std::tuple<unsigned long, CarriedCurvesConfiguration> compute_intersection_number(
    ...
    static TrainTrack random_trivalent_train_track(URBG& rng, size_t switches_count,
                                                   const TrainTrackOptions& options = TrainTrackOptions());
